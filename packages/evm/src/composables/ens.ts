...
function useEnsBase(
  ...
  const mode = computed<EnsMode>(() => toValue(options.mode) || evmConfig.ens?.mode || 'indexer')
  const indexerUrls = computed(() => evmConfig.ens?.indexerUrls || [])
  const id = computed(() => toValue(identifier))

  return useQuery({
    queryKey: ['ens', tier, id] as const,
    queryFn: async () => {
      ...
      return ensCache.fetch(`ens-${tier}-${currentId}`, () =>
        resolve(currentId, strategies, indexerUrls.value, wagmiConfig as Config, chainKeys),
      )
    },
    enabled: () => !!id.value,
    staleTime: 5 * 60 * 1000,
